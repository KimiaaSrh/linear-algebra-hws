...
while(loopcounter<int(array[0])):
    numrows=int(array[fileHeader])
    numcols=int(array[fileHeader+1])
    A=[[0 for x in range(numcols)] for y in range(numrows)]
    y=[[0 for x in range(1)] for y in range(numrows)]
    counter=0
    fileHeader+=2
    while(counter<numrows):
        counter2=0
        while(counter2<numcols):
            A[counter][counter2]=array[fileHeader]
            fileHeader+=1
            counter2+=1
        counter+=1
    counter=0
    while(counter<numrows):
        y[counter][0]=array[fileHeader]
        counter+=1
        fileHeader+=1
